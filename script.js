...
function renderBoard(board) {
  ...
  document.title = "Zuddl-Assignment";
...
function addBoard() {
  ...
  if (appData.boards.length >= 512)
    return createAlert("Max limit for boards reached."); // or if there are already too many boards
  ...
  generateElement() {
    ...
    _newButton.addEventListener("click", () => {
      ...
      let _item = new Item(
        _inputValue,
        null,
        getBoardFromId(this.parentBoardId).uniqueID(),
        this.id
      );
      this.addItem(_item);
      _newInput.value = "";
...
class Board {
  ...
  addCard() {
    ...
    if (!_cardTitle) _cardTitle = `Untitled Card`;
...
function loadData() {
  ...
  if (_data) {
    ...
    appData.currentBoard =
      _appData.currentBoard >= 0 ? appData.currentBoard : 0;
    ...
    let _defaultBoard = new Board("Resources", "b0", { theme: null });
